  ...
  Future<List<dynamic>> fetchFaqs() async {
    try {
      ...
      if (response.statusCode == 200) {
        print(
            "Datos recibidos: ${response.body}"); // Log para confirmar los datos recibidos
        return jsonDecode(response.body);
      } else {
        print(
            "Error al cargar FAQs: ${response.statusCode}"); // Log en caso de error en la solicitud
        throw Exception('Failed to load FAQs');
      }
    } catch (e) {
      print("Excepción al cargar FAQs: $e"); // Log de la excepción
      throw Exception('Failed to load FAQs');
    }
  }
